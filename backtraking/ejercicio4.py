...
def generar_parentesis_corchetes(n):
    ...
    def backtrack(cadena, abiertos_p, cerrados_p, abiertos_c, cerrados_c, pila):
        ...
        if abiertos_p < n:
            backtrack(cadena + "(",abiertos_p + 1,cerrados_p,abiertos_c,cerrados_c,pila + "(")

        if abiertos_c < n:
            backtrack(cadena + "[",abiertos_p,cerrados_p,abiertos_c + 1,cerrados_c,pila + "[")

        if cerrados_p < abiertos_p and pila and pila[-1] == "(":
            backtrack(cadena + ")",abiertos_p,cerrados_p + 1,abiertos_c,cerrados_c,pila[:-1])

        if cerrados_c < abiertos_c and pila and pila[-1] == "[":
            backtrack(cadena + "]",abiertos_p,cerrados_p,abiertos_c,cerrados_c + 1,pila[:-1])
